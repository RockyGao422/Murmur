  ...
  switch (action) {
    ...
    case 'getStatus': {
      ...
      const pendingCount = todaySessions.filter(
        (s) => s.status === SessionStatus.NEEDS_COMPLETION || s.status === SessionStatus.SUSPECTED_ABANDONED
      ).length;
    ...
    case 'getTodayStats': {
      ...
      for (const s of sessions) {
        toolDist[s.toolName] = (toolDist[s.toolName] || 0) + s.duration;
      }
 ...
 */
function tryConnectNativeMessaging() {
  try {
    if (typeof nativeMessaging !== 'undefined') {
      nativeMessaging.connect();
