...
async function matchEvent(rawEvent, toolState) {
  ...
  const urlPattern = rawEvent.urlPattern;

  // No domain → can't match
  if (!domain) {
    return {
      tool: null,
      confidence: 0,
  ...
  // Phase 2: URL pattern match (match domain against pattern, or urlPattern if available)
  for (const tool of webTools) {
    for (const pattern of tool.url_patterns) {
      const testUrl = urlPattern ? `https://${urlPattern.replace('/*', '')}` : `https://${domain}/`;
      if (urlMatchesPattern(testUrl, pattern)) {
        return {
          tool: tool,
